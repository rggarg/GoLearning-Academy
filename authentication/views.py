        ...
        if not User.objects.filter(username=username).exists():
            if not User.objects.filter(username=username).exists():
                ...
                email_body = 'Hi, ' + user.username + \
                    '\n To complete your signup please verify your email.\n' + activate_link + '\n\n\n Thanks,\n GoLearning Academy \n'
                email = EmailMessage(
                    email_subject,
                    email_body,
                ...
                messages.success(
                    request, 'Account created successfully.Please visit your email and activate your account.')
        ...
        if username and password:
            ...
            if user:
                if user.is_active:
                    ...
                    messages.success(
                        request, 'Hey! '+user.username+' Welcome to GoLearning Academy')
...
class RequestPasswordResetEmail(View):
    ...
    def post(self, request):
        ...
        if user:
            ...
            email_body = 'Hi, ' + user.username + \
                '\n You recently requested to reset the password for your GoLearning Academy account. Click the link below to proceed. \n' + \
                reset_url + '\n\n If you did not request a password reset, please ignore this email.  \n\n\nThanks,\n GoLearning Academy team'
            email = EmailMessage(
                email_subject,
                email_body,
                'noreply@semycolon.com',
...
class CompletePasswordReset(View):
    ...
    # if yes then user get a error otherwise got option to reset the password
...
class PasswordValidationView(View):
    ...
    def post(self, request):
        ...
        if len(password) < 8:
            return JsonResponse({'password_error': 'Password length should be atleast of 8 character'}, status=400)
        if len(password) > 12:
            return JsonResponse({'password_error': 'Password length should not be greater than 12 character'}, status=400)
        if not('@' in password or '_' in password or '$' in password or '#' in password):
            return JsonResponse({'password_error': 'Password should have at least one of the symbols @$#_'}, status=400)
        if not any(char.isupper() for char in password):
            return JsonResponse({'password_error': 'Password should have atleast one uppercase letter'}, status=400)
        if not any(char.islower() for char in password):
            return JsonResponse({'password_error': 'Password should have atleast one lowercase letter'}, status=400)
        if not any(char.isdigit() for char in password):
            return JsonResponse({'password_error': 'Password should have atleast numeral value'}, status=400)
